...
function generatePDF() {
  // Bill number logic: store and increment in localStorage
  let billNo = localStorage.getItem('wuroud_bill_no');
  if (!billNo) {
    billNo = 'AA1';
  } else {
    // Increment bill number (e.g., AA1 -> AA2)
    const prefix = billNo.match(/^[A-Z]+/)[0];
    const num = parseInt(billNo.match(/[0-9]+$/)[0], 10) + 1;
    billNo = prefix + num;
  }
  localStorage.setItem('wuroud_bill_no', billNo);
  ...
  doc.setFontSize(11);
  y += 18;
  doc.text('PUTHIRIKAL, PARAPPANAGADI', pageWidth/2, y, { align: 'center' });
  y += 14;
  ...
  doc.text(`Bill No: ${billNo}`, pageWidth - 140, y);

  // Table
  y += 18;
  const head = [['Item', 'Qty', 'Amt']];
  ...
  doc.autoTable({
    ...
    startY: y + 6,
    theme: 'plain',
    headStyles: { fontStyle: 'bold', fontSize: 11, halign: 'center', lineWidth: 0.5, lineColor: [0,0,0] },
    styles: { font: 'helvetica', fontSize: 11, cellPadding: 4, halign: 'center' },
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(11);
  doc.text('Sub Total', 320, y);
  doc.text(safeRupee(total), pageWidth - 80, y, { align: 'right' });

  // Medium space before TOTAL
  y += 28;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.text('E & O E', pageWidth - 80, y, { align: 'right' });

  doc.save(`Wuroud-bill-${billNo}.pdf`);
}
