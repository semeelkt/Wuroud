...
addProductBtn.addEventListener('click', async () => {
  ...
  const docRef = await addDoc(productsCol, { name, price, category: pCategory.value, img: pImg.value.trim() });
...
function updateCart() {
  ...
  cartList.innerHTML = cart.map((it,i)=>{
    total+=it.price;
    return `<li>${it.name} - ₹${it.price} <button class="remove-btn" onclick="removeFromCart(${i})">❌</button></li>`;
  }).join('');
  cartTotal.textContent = cart.length ? `Total: ₹${total}` : 'No items selected.';
}
clearCartBtn.addEventListener('click', () => { if(cart.length && confirm('Clear cart?')) { cart=[]; updateCart(); } });
...
// ===== OPEN APP =====
function openApp(){ overlay.classList.add('hidden'); appDiv.classList.remove('hidden'); fetchProducts(); updateCart(); }
prepareAuthUI();
